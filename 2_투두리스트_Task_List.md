# 📋 TOC B2B 멀티테넌시 계측 모니터링 시스템 구축 투두리스트 (Task List)

본 문서는 프로젝트 진행에 따른 세부 구현 요구사항의 구현 이력과 완료 검증 상태를 나타내는 작업 진척 보고서입니다.

---

## 1. 종합 진척 상황 (Overall Progress)

* **종합 진척률**: **100%**
* **현재 목표**: 다중 사이트 확산 대비 — 지점 식별자(site_id) 전파 결함 수정, 대시보드 단일 지점 하드코딩 제거, 저장소 관리 체계(문서·비밀정보·추적 파일) 정비 완료.

---

## 2. 세부 개발 Task 현황

### Phase 1~5: 로컬 Python Uploader 및 Google Sheets 프로토타입 개발 (완료 100%)
* [x] 구글 Sheets 연동 데이터 전송 및 GUI 업로더 인터페이스 개발 완료 (1차 프로토타입)
* [x] 다중 기기 식별 및 고객사별 대시보드 물리 격리 시뮬레이션 검증 완료

### Phase 6: Supabase 데이터베이스 마이그레이션 및 차트 범위 수정 (완료 100%)
* [x] **Supabase DB 연결 자격 증명 연동 및 데이터 테이블 적재 마이그레이션**
* [x] **대시보드 트렌드 차트 표시범위 기본값을 기존 7일에서 24시간(24h)으로 최적화**

### Phase 7: 3단계 알림 고도화 및 SMTP 이메일 발송 연동 (완료 100%)
* [x] **주의(노랑) / 경고(빨강) 3단계 등급 테이블 시각화 구현**
* [x] **별도 스키마 변경 없이 `toc_alert_high` JSON 필드 내 설정값 일괄 패키징 및 웹 저장 연동**
* [x] **`wsmtp.ecounterp.com` SMTP 서버 연동 및 채널별 1시간 쿨다운 경고 메일 발송 로직 구현**
* [x] **알람 전송 인터페이스 추상화 (`alerts/` 패키지화)**
* [x] **카카오톡/MMS 대비 이메일 발송 모듈화 (`EmailAlertSender` 도입 및 설정 연동)**
* [x] **`?admin` 경로 분기 및 접속용 패스코드(Passcode: `850`) 보안 검증 게이트 구현**
* [x] **테스트용 수동 모의 데이터(`[MOCK TEST DATA]`)의 일반 사용자 대시보드 노출 차단 필터 구현**
* [x] **웹 UI 테스트 메일 발송 비동기 플래그(`trigger_test_email`) 핸들러 및 파이썬 폴링 리셋 로직 연동**
* [x] **설정 저장 시 5000 미만 임계치 설정에 대한 경고 확인창(confirm)을 완전히 제거하여, 모든 범위(0 이상)의 커스텀 경고 기준값을 알림 없이 즉시 저장할 수 있도록 개선**

### Phase 8: 단일 테이블 통합 및 텔레그램 무료 알림 연동 (완료 100%)
* [x] **모든 장비가 단일 테이블(measure_logs)을 공유하여 데이터베이스 자원을 효율화하는 싱글 테이블 아키텍처 구현**
* [x] **API 쿼리 및 증분점 탐색 시 `Device_ID` 필터링 주입 적용 완료**
* [x] **텔레그램 API 연동 발송 모듈(`TelegramAlertSender`) 신규 구축 완료**
* [x] **기존 스키마 DDL 수정 없이 `toc_alert_high` JSON 필드 내에 Telegram Chat ID 및 테스트 트리거(`trigger_test_telegram`) 패키징 저장**
* [x] **웹 대시보드(App.jsx / LegacyApp.jsx)에 Chat ID 입력창 및 즉시 테스트 메시지 발송 신호 연동 완료**

### Phase 9: V2 스키마 설계 및 지점 식별자 직접 매립 아키텍처 (완료 100%)
* [x] **Supabase 신규 테이블 `site_config_v2` 및 `measure_logs_v2` 구조 정의 및 SQL DDL 배포 가이드라인 수록**
* [x] **업로더 프로그램(gui_uploader.py) 내 `Site_ID` (지점 코드) 및 `Device_ID` (장치명) 개별 로드 및 API 연동**
* [x] **서버 증분 탐색(`query_server_latest_datetime`) 시 `Site_ID` 필터 기준으로 조회하여 데이터 정합성 보장**
* [x] **웹 대시보드(App.jsx / LegacyApp.jsx)의 데이터 조회(`loadData`), CSV 다운로드, 모의 데이터 쿼리 전체를 `Site_ID` 기준으로 필터 조회하도록 V2 마이그레이션**
* [x] **V5.0 GUI 업로더 컴파일 (`gui_uploader_v5.0.exe`) 및 배치파일 연동 완료**

### Phase 10: 고객 알람 자가 등록 및 관리자 삭제 권한 분리 (완료 100%)
* [x] **고객이 직접 이메일/텔레그램 알림 수신처(이름, 구분, 수신 주소)를 등록할 수 있는 대시보드 공개 양식 구현**
* [x] **관리자 설정 모달(패스코드 gated)에 등록된 수신자 정보(이름, 형태, 주소) 매핑 테이블 렌더링**
* [x] **관리자 모드(`?admin` 파라미터 활성화)에서만 `🗑️ 삭제` 버튼이 작동하도록 권한 통제 적용**
* [x] **자가 등록 및 삭제 동작 시 `receivers` JSON 배열 변경이 실시간으로 `alert_emails` 및 `telegram_chat_ids` 콤마 분리형 DB flat 필드에 자동 결합 동기화되도록 처리하여 현장 PC 업로더 프로그램 호환 유지**

### Phase 11: 업로더 프로그램 UI 현대화 및 지점 한글명 설정 폼 연동 (완료 100%)
* [x] **업로더 GUI 프로그램에 Segoe UI/맑은 고딕 글꼴 및 프리미엄 다크 네이비/시안 테마(패밀리룩) 적용**
* [x] **원형 캔버스 LED 활성 인디케이터(ACTIVE/PAUSED 글로우 컬러 분기) 적용**
* [x] **상세 설정 숨기기/펼치기 폴더블(collapsible) 레이아웃 및 원클릭 모든 설정 영구 저장 파일 write 연동**
* [x] **웹 대시보드 설정 모달 내 "🏢 지점 한글명 설정" 텍스트 입력창 신설 및 DB PATCH 동기화 연동**
* [x] **배포 패키지 uploader_config.json 내 Supabase URL, Key, default Telegram token 기본 기입 및 v5.1 리비전 패키징 완료**

---

### Phase 12: 업로더 설정 단순화 및 동기화 주기/기동 상태 설정 구현 (완료 100%)
* [x] **불필요한 지점 식별자 코드(Site ID) 입력 필드를 UI에서 제거하여 환경 단순화** (설정 파일에서는 계속 로드/저장 유지)
* [x] **'사이트 이름 (지점 한글명)' 라벨을 직관적인 '사이트 이름 (Site Name)'으로 변경**
* [x] **'자동 동기화 주기 (분 단위)' 입력 필드를 신설하여 분 단위 세밀한 주기 설정 및 초 단위 변환 저장 구현**
* [x] **'시작 시 자동 동기화 활성화' 체크박스를 통해 기동 시 일시정지(`is_paused`) 또는 즉시 구동 상태 지정 및 저장 연동**
* [x] **실시간 큐 이벤트 및 새로고침 주기에서 사이트 이름(`site_name`) 값이 GUI와 상호 즉시 동기화되도록 수정**
* [x] **배포 패키지 디렉토리(`계측기_PC_배포패키지_v5.1`) 내 단일 실행 파일 및 uploader_config.json 설정 구성 완료**

---

### Phase 13: 현장 네트워크 불안정 대응 및 커넥션 핸드셰이크 예외 완화 (완료 100%)
* [x] **공통 HTTPS API 호출용 `make_supabase_request` 헬퍼 메소드 구현**
* [x] **일시적 망 유실/지연 극복을 위한 최대 3회 자동 재시도(Retry) 메커니즘 통합**
* [x] **SSL 핸드셰이크 타임아웃 방지를 위해 기존 5~8초로 한정되어 있던 API 타임아웃 제한을 30초로 완화**
* [x] **`gui_uploader.py` 내 모든 GET/POST/PATCH Supabase 연결 부위를 신규 헬퍼로 전면 마이그레이션 및 바이너리 재빌드 완료**
* [x] **Supabase `measure_logs_v2` 테이블 스키마에 정의되지 않은 `MAXR` 컬럼을 JSON 전송 페이로드에서 사전 제외하여 HTTP 400 Bad Request 에러 완벽 해결**
* [x] **Tkinter UI 그리드 패킹 순서를 최적화하여 하단 `[즉시 동기화]` 및 `[일시정지]` 컨트롤 버튼 프레임이 항상 노출되도록 보장**

---

## 3. 최종 품질 및 동작 검증 결과
1. **차트 기본 범위 검증**: React 앱 로드 시 `App.jsx`에서 `24h`가 기본 활성화 상태로 로드되어, 대량 데이터 조회 시의 초기 로딩 부하를 크게 감소시켰습니다.
2. **이메일 발송 상태**: python uploader에서 설정된 임계치 초과 시 ecounterp SMTP를 통해 경고 메일이 발송되는 코어 모듈 동작을 확인했습니다.
3. **단일 테이블 및 텔레그램 푸시 알림 검증**:
   * `VITE_USE_SINGLE_TABLE` 환경변수가 활성화되면 대시보드에서 단일 테이블(`measure_logs` 등)에 `Device_ID` 필터를 적용하여 데이터를 로드합니다.
   * `TelegramAlertSender` 모듈이 오류 없이 컴파일되었으며, 10초 주기의 백그라운드 스레드 폴링을 통해 웹에서 요청한 `trigger_test_telegram` 신호를 성공적으로 캐치하여 초기화합니다.
4. **V2 스키마 및 Site_ID 연속성 연동 검증**:
   * `site_config_v2` 및 `measure_logs_v2` 통합 데이터 테이블 조회를 구현하여, PC 교체 시에도 과거 데이터가 한 화면에 매끄럽게 연결되는 아키텍처 구축을 검증 완료했습니다.
5. **고객 자가등록 및 관리자 제어 검증**:
   * 대시보드 메인 화면의 `🔔 알림 수신 등록` 버튼을 클릭하여 누구나 이름과 수신처를 입력해 자가 신청할 수 있습니다.
   * 이 정보는 `receivers` JSONB 필드로 구조화 관리되며, `?admin` 모드로 진입한 관리자만 삭제 버튼을 보고 제어할 수 있도록 안전하게 인가 격리되었습니다.
   * 저장/삭제 시 SQLite 기반 현장 PC uploader가 참조하는 flat 컬럼 데이터(`alert_emails`, `telegram_chat_ids`)가 콤마로 즉시 변환 동기화되어 이전의 하위 호환성을 그대로 보존합니다.
6. **업로더 v5.1 UI 현대화 및 지점 한글명 관리 검증**:
   * 현장 PC uploader GUI 화면을 프리미엄 다크 네이비 테마로 리뉴얼하고, 접이식 상세 설정 윈도우 및 영구 저장 기능을 완성했습니다.
   * 대시보드 어드민 설정 폼에서 지점 한글명을 기입하고 저장 시, Supabase의 `site_name`이 즉시 갱신되어 상단 헤더에 동적 연동 표시됨을 확인했습니다.
   * 배포 패키지 v5.1 내에 SUPABASE URL/Key 및 텔레그램 봇 토큰 값을 매립 동봉하여 현장 납품 및 구동 준비 상태를 완료했습니다.
7. **업로더 설정 최적화 및 간소화 검증**:
   * 불필요했던 Site ID 입력 필드를 화면에서 완전히 숨겨 사용자 실수를 방지하고, 사이트 이름 표기를 직관적으로 일원화했습니다.
   * 동기화 주기(분 단위)와 기동 시 자동 동기화 체크박스를 UI 전면 배치하여, 사용자가 현장 네트워크 환경에 맞게 간편하게 주기를 튜닝하고 일시정지 상태 기동 여부를 클릭 한 번으로 지정할 수 있음을 검증했습니다.
8. **네트워크 장애 감쇄 및 핸드셰이크 복원력 검증**:
   * 판넬 PC 등 산업용 고지연 내부망에서 흔히 일어나는 SSL 핸드셰이크 타임아웃 문제를 해결하기 위해 타임아웃을 30초로 설정하고, 1.5초 간격으로 최대 3회까지 재시도하는 로직의 복원력을 실시간 로그 기록을 통해 완벽히 가동 중임을 확인했습니다.
9. **Supabase DDL 불일치 제거 (HTTP 400 해결)**:
   * 로컬 SQLite 뷰에는 존재하지만 Supabase `measure_logs_v2` DDL에는 선언되지 않은 `MAXR` 항목을 JSON 데이터 페이로드에서 분리 필터링하여 `400 Bad Request` 에러를 최종 퇴치했습니다.
10. **컨트롤 패널 레이아웃 클리핑 해결**:
    * 텍스트 로그창(`logger_frame`)이 스크린 높이를 넘겨 하단 버튼을 덮는 스크롤바 클리핑을 막기 위해, Bottom Control Frame을 `side=tk.BOTTOM`으로 사전 할당 배치하여 상시 조작 가능 상태를 확인했습니다.
11. **다중 기기 설정 격리 및 덮어쓰기 방지 검증**:
    * 배포 패키지 내 설정 파일의 기기 ID를 `auto`로 리셋하여, 최초 구동하는 모든 PC가 고유의 물리 호스트네임을 기반으로 원격 설정 행을 분리 생성하도록 구현했습니다.
    * 업로더 GUI 화면에 **지점/기기 식별자 ID (Site ID - 고유값)** 입력 필드를 노출하고 활성화하여, 사용자가 각 계측기 PC별로 자유롭게 고유한 키(예: `TOC-260706-03` 등)를 할당 및 갱신해 개별 지점명, 패스코드, 알림 대상자 풀을 완전히 격리 관리할 수 있음을 검증했습니다.

---

### Phase 14: 다중 기기 설정 덮어쓰기 방지 및 지점 식별자 직접 편집 지원 (완료 100%)
* [x] **배포용 템플릿 uploader_config.json 내 site_id 및 site_name 기본값을 auto로 전환하여 다중 기기 자동 식별 보장**
* [x] **업로더 GUI 내에 지점/기기 식별자 ID (Site ID - 고유값) 입력 및 변경 제어창 신설 (state="normal")**
* [x] **설정 저장 시 site_id 값 유효성 검사 적용 및 Supabase site_config_v2의 해당 식별자 갱신 로직 연동**
* [x] **바이너리 재빌드 (gui_uploader_v5.1.exe) 및 배포 디렉토리 업데이트 완료**

---

### Phase 15: Supabase V3 기기 설정 테이블 신설 및 대시보드/업로더 다중 기기 독립 관리 (완료 100%)
* [x] **기기 고유 ID (device_id)를 기본키로 사용하는 device_config 설정 테이블 신설 스키마 DDL 작성**
* [x] **웹 대시보드에서 URL 파라미터(?site= 및 ?device=)에 따라 데이터 조회(Device_ID 혹은 Site_ID 기준) 및 설정 동기화 분기 쿼리 구현**
* [x] **업로더 프로그램의 REST API 조회 및 갱신 대상을 신설 device_config 테이블로 전면 변경하고 device_id 고유키 기준으로 동작 구현**
* [x] **업로더 증분점 데이터 최종 시각 쿼리를 Site_ID가 아닌 Device_ID 기준으로 정밀화하여 다중 기기 충돌 유실 방지**
* [x] **업로더 GUI 내에 Site ID와 Device ID를 둘 다 명시적으로 입력하고 수동 편집할 수 있도록 UI 그리드 재구조화**
* [x] **최종 소스 반영 바이너리 (gui_uploader_v5.1.exe) 재빌드 및 배포 패키지 업데이트 완료**

---

### Phase 17: Device ID readonly 및 Caution/Warning 2단계 임계치 설정 고도화 (완료 100%)
* [x] **계측기 GUI Uploader 내 Device ID 입력 필드를 `state="readonly"`로 변경하여 오기 방지 및 복사(Ctrl+C) 가능 조치**
* [x] **대시보드 관리자 설정 화면 및 모달창에 주의(Caution) 및 경고(Warning) 2단계 기준 입력 필드 및 알람 발송 레벨 셀렉트 박스 신설**
* [x] **웹 대시보드 실시간 데이터 테이블 및 어드민 내역의 TOC 수치 글자 색상 분기 렌더링 (주의: 주황색, 경고: 빨간색)**
* [x] **SQLite/Supabase uploader 연동 알람 체크 스레드 내 주의/경고 트리거 기준 및 alert_level 매핑 분기 호환 패치 완료**
* [x] **신규 바이너리 컴파일 (`gui_uploader_v5.2.exe`) 및 배포 패키지(`계측기_PC_배포패키지_v5.2`) 제작 완료**
* [x] **대시보드 접속 주소 파라미터(?site= 또는 ?device=) 입력값 유효성 검사 및 미등록 기기 접속 전면 차단(보안 강화)**
* [x] **업로더 최초 기동 시 `site_id` 또는 `site_name`이 `"auto"`인 경우 강제 일시정지(`is_paused = True`) 및 Supabase 기기등록/업로드 원천 차단(Fail-safe)**
* [x] **일시정지 상태 시 10초 주기 테스트 알림 감지 등 백그라운드 Supabase API 쿼리 요청 전면 중단 처리**
* [x] **GUI 설정 저장 시 `site_id` / `site_name`이 auto가 아니면 `is_auto_config` 플래그 자동 해제 연동**

---

### Phase 18: Vercel 직접 텔레그램 발송 튜닝, 접속 패스코드 변경 및 수신처 삭제 권한 완화 (완료 100%)
* [x] **Vercel(웹 브라우저)에서 직접 텔레그램 Bot API를 호출하여 즉시(0초 딜레이) 테스트 메시지를 보내는 `sendTelegramAlert` 연동**
* [x] **알림 수신인 등록/삭제 시 텔레그램 봇이 환영/작별 메시지를 즉시 발송하도록 튜닝**
* [x] **설정창(설정 모달) 내부의 수신자 삭제(`🗑️ 삭제`) 권한을 어드민 여부와 무관하게 상시 작동하도록 완화**
* [x] **설정 모달 및 어드민 설정 카드 내에 대시보드 접속 패스코드(비밀번호) 변경 텍스트 입력창 신설 및 DB PATCH 동기화**
* [x] **설정 모달 및 어드민 설정 카드 내에 자체 텔레그램 봇 토큰 입력 필드 추가 및 `toc_alert_high` JSON 필드 연동 저장**
* [x] **계측기 GUI Uploader 백그라운드 스레드에서 Supabase `device_config`를 읽을 때 웹에서 설정한 `telegram_bot_token`을 자동 감지하여 동기화**

---

### Phase 19: 설정창 UI 레이아웃 콤팩트 고도화 및 텔레그램 봇 자가 등록 (/등록) 구현 (완료 100%)
* [x] **대시보드 메인 헤더의 `🔔 알림 수신 등록` 버튼을 삭제하고, 설정 모달 내부 수신자 목록 타이틀 옆에 수신처 추가 단추 매립**
* [x] **설정 모달 및 어드민 설정 카드 내의 투박한 채널 테이블을 반응형 Grid Layout 기반의 콤팩트 카드형 배치로 전환**
* [x] **설정 모달창의 가로폭을 800px로 넓히고, 모달 몸체에 세로 스크롤(`max-height: 90vh`, `overflow-y: auto`) 적용하여 사용성 강화**
* [x] **설정 모달 내에 중복 노출되어 세로 공간을 낭비하던 이메일/봇토큰/챗ID 입력창들을 전면 삭제하고 등록인 목록만 깔끔하게 유지**
* [x] **일반 공개 등록 모달에서 이메일 등록 옵션을 삭제하고 오직 텔레그램 가이드와 Chat ID 입력창만 활성화되도록 단독 모드 고정**
* [x] **사용자가 텔레그램 봇 대화방에서 `/등록 [디바이스ID] [사용자명] [패스코드]`를 입력하면, Vercel Serverless Function(Edge API Webhook)이 즉시 기기 비밀번호를 검증하고 Supabase DB에 수신자를 자동 자가등록하는 서버리스 Webhook 아키텍처 구현 완료**
* [x] **로컬 파이썬 Uploader(`gui_uploader.py`) 내에 임시로 추가되었던 10초 주기 텔레그램 getUpdates 폴링 백그라운드 스레드 및 메소드 일체 삭제 및 원복(경량화 유지)**
* [x] **일반 공개 알림 수신 등록 모달 하단의 텔레그램 Chat ID 확인 가이드 3단계 설명 문구 수정 (User info 및 라스코리아 알람 봇 검색 방법 연계)**
* [x] **텔레그램 알림 봇으로 `/start` 메시지 전송 시 환영 메시지 및 자가등록 방법(`/등록`) 자동 안내 메시지 회신 로직 Vercel Serverless Webhook에 구현 완료**
* [x] **실시간 경보(주의/경고) 판단 및 텔레그램 발송 엔진의 Vercel 서버리스 완전 이관 완료**: Uploader 로컬 알람 로직 전면 제거 및 /api/send-alert 전송용 클라우드 Webhook 트리거 셋업 명세화 완수
* [x] **트렌드 차트(Recharts) 내 이중 Y축(보조축) 채널 선택 기능 구현 및 설정 UI 레이아웃 사소한 문구 정리 완료** 및 데이터베이스 저장 기반의 영구 채널 축 설정(기본축/보조축 지정)으로 전환 완료

---

### Phase 20: 텔레그램 스마트 자가등록 및 대시보드 UI 장비 ID 개선 (완료 100%)
* [x] **설정 모달 창의 알림 수신자 헤더 옆 장비 ID 노출부에 폴백(Fallback) 감지 추가**: 로그 상의 Device_ID가 사이트 ID와 같으면 DB 설정의 `device_id` 필드를 노출시켜 장비 번호를 정상 노출하도록 조치
* [x] **텔레그램 알림 봇 등록 Webhook (`/api/telegram-webhook.js`) 쿼리에 OR 다중 검색 튜닝**: 사용자가 기기 ID 또는 사이트 ID 중 어느 것을 입력하더라도 매칭 성공하도록 PostgREST `or` 쿼리 적용
* [x] **실서비스 도메인(`toc-850-web-dashboard.vercel.app`) 및 테스트 스테이징 서버 최신 리비전 통합 릴리즈 및 원격 Git 릴리즈 배포 완수**

---

### Phase 21: 차트 기본축 및 보조축 스케일 범위 지정 및 동적 확장 (완료 100%)
* [x] **기본축(Left YAxis) 및 보조축(Right YAxis) 스케일 범위 자동 설정**: 0을 하한으로 하고 해당 축에 설정된 표시 채널의 경고치(Warning Threshold) 중 최대값을 상한으로 하는 기본 스케일 산정 알고리즘 구축
* [x] **플롯 범위 초과 시 동적 확장(Dynamic Extension) 구현**: 플롯 범위 데이터 중 경고치 상한을 초과하는 데이터(`dataMax > maxWarning`) 발생 시 데이터 최대값(+ 약 5% 여유)으로 상한 확장, 0 미만의 음수 데이터(`dataMin < 0`) 발생 시 데이터 최소값으로 하한 확장 연산 구현
* [x] **기본축/보조축 독립 YAxis domain 및 수동 줌 슬라이더 제어 통합 연동**: Recharts `<YAxis>` 및 Y 줌 슬라이더 컨트롤 완벽 호환 보장


---

### Phase 22: 지점 식별자(site_id) 전파 결함 수정 및 저장소 관리 체계 정비 (완료 100%)

#### 22-1. site_id가 대시보드에 반영되지 않던 결함 수정 (업로더)
* [x] **원인 규명**: Supabase 실데이터 대조로 `measure_logs_v2.Site_ID`에는 신규 지점 ID가 반영되나 `device_config.site_id`는 최초 등록값(대개 PC 호스트명)에 동결되는 현상 확증
* [x] **`save_config()`의 site_id 미전송 결함 수정**: 기존 `bg_update_site_name_on_supabase()`는 PATCH payload에 `site_name`만 담고 `site_id`를 누락했음
* [x] **죽은 코드 활성화**: `auto_register_device_config()` / `fetch_device_config()`가 정의부와 상호 재귀 외에 어떤 호출 지점도 없어, 신규 사이트는 `device_config` 행 자체가 생성되지 않던 문제 해소
* [x] **`sync_device_config_to_supabase()` 신설**: `device_id` 기준 PATCH(`Prefer: return=representation`으로 매칭 확인) 후 0건이면 POST 하는 멱등 upsert 구현
* [x] **호출 지점 2곳 연결**: 설정 저장 시 + 매 동기화 시작 시 호출 → 현장에서 저장 버튼 1회로 기존 오등록 행이 자동 교정되도록 조치
* [x] **Fail-safe 가드 무력화 수정**: `load_config()`가 `"auto"`를 호스트명으로 먼저 치환한 뒤 가드가 다시 `"auto"` 문자열을 검사하여 절대 참이 되지 않던 결함을 `is_auto_config` 플래그 기반으로 교정
* [x] **`build_device_config_url()` 헬퍼 도입**: `/rest/v1` 중복 보정 로직 3중 중복 제거

#### 22-2. 다중 사이트 확산 저해 요인 제거 (대시보드)
* [x] **`VITE_SUPABASE_TABLE` 하드코딩 기본값(`"Samyang_Incheon"`) 및 `siteSearchTerm` 폴백 제거**: 파라미터 누락 시 타 지점 사용자에게 기존 고객사 데이터가 노출될 수 있던 위험 제거
* [x] **죽은 `site_config_v2` 폴백 경로 제거**: 해당 테이블은 존재하지 않아(`PGRST205`) 항상 404 후 접속 차단으로 직행하던 경로를 **측정 데이터 유무 진단 조회**로 대체
* [x] **접속 차단 화면 진단 안내 추가**: 데이터는 수신되나 `device_config`가 없는 경우 "업로더에서 설정 저장을 1회 실행" 안내를 화면에 표시
* [x] **한 지점 다중 기기 대응**: 기존 `confList[0]` 무조건 선택을 개선하여 `?device=` 지정 시 해당 기기 행, 미지정 시 `site_id` 정확 일치 행 우선 선택
* [x] **미정의 변수 `deviceIds` 참조 제거**: 테스트 메일 발송 버튼 클릭 시 ReferenceError로 크래시하던 잠복 결함 수정

#### 22-3. GitHub 노출 자격 증명 제거
* [x] **노출 범위 전수 조사**: service_role 키가 배포패키지 config 외에 `TOC850_Company_Handoff/gui_uploader.py` 하드코딩, 과거 `App.jsx` / `scratch_test.js`에도 존재했음을 확인. SMTP 비밀번호·텔레그램 토큰은 회의록 및 통합명세서에도 존재
* [x] **전체 저장소 백업**: `git bundle create full-repo-backup.bundle --all` (11.9MB, 전체 ref 보존)
* [x] **작업트리 정리**: 소스/문서 내 비밀정보 플레이스홀더 치환 및 배포패키지 `uploader_config.json` 추적 해제(디스크 파일은 현장 사용을 위해 유지)
* [x] **`git filter-repo --replace-text`로 전체 117개 커밋 재작성** 후 `origin/main` force push (`a39d728` → `c4f1f3e`, `--force-with-lease` 적용)
* [x] **원격 히스토리 전수 재검사**: service_role 키 / SMTP 비밀번호 / 텔레그램 토큰 **잔여 0건** 확인

#### 22-4. 저장소 관리 체계 정비
* [x] **`.gitignore` 전면 재작성**: 섹션 정리, `**/uploader_config.json` 추가, `.env.example` 커밋 유지 negation 추가, "이미 추적 중인 파일은 규칙 추가만으로 제외되지 않음" 경고 주석 명시
* [x] **불필요 추적 파일 해제(디스크 유지)**: `.vercel/`, `gui_uploader_v5.1.exe`(11MB), `chrome_debug.log`, Handoff `mock_google_sheet.csv`(3.5MB)
* [x] **`docs/ARCHITECTURE.md` 신설**: 시스템 구조도, `site_id`/`device_id` 식별 체계, site_id 전파 경로, 신규 사이트 온보딩 절차, 증상별 트러블슈팅 표, 알려진 제약 및 보안 현황 수록
* [x] **`README.md` 재작성**: 실제 디렉터리 구조 및 "오해하기 쉬운 항목"(`uploader.py` 미사용, `TOC850_Company_Handoff/` 현행 아님) 명시
* [x] **`dashboard/.env.example` 갱신**: service_role 키 사용 금지 경고 및 URL 파라미터 기반 지점 선택 규칙 명시

#### 22-5. 후속 과제 (미착수)
* [ ] **자격 증명 재발급**: 노출 이력이 있으므로 유출 간주. Supabase API 키 / SMTP 비밀번호 / 텔레그램 봇 토큰 재발급 및 전 현장 `uploader_config.json` 갱신 필요
* [ ] **업로더 최소 권한 전환**: service_role → anon 키 + `measure_logs_v2` INSERT / `device_config` UPSERT 전용 RLS 정책
* [ ] **`sites` 테이블 분리**: 지점 정보(site_id/site_name/passcode/임계값)가 기기 행마다 중복 저장되는 구조적 결함 해소 (스키마 변경 승인 필요)
* [ ] **`device_config.is_active` 컬럼 부재**: 대시보드가 `conf.is_active !== false`로 읽어 항상 `true`. 지점 비활성화 기능 무효
* [ ] **`App.jsx` 약 2,900줄 단일 파일 컴포넌트 분리**
* [ ] **`TOC850_Company_Handoff/` 사본 처리 방침 결정** (보존 / 삭제)

### 23. 신기폐수처리장 지점 식별자 통합 및 과거 데이터 병합 (2026-08-13)

#### 23-1. 원인 규명
* [x] **증상 확증**: 최신 데이터 미표시 + 사이트 이름으로만 접속되는 현상을 Supabase 실데이터 조회로 재현. `device_config.site_id`가 호스트명(`TMSTOC-250701-01`)에 동결되어 접속 판정(site_id **또는** site_name)은 통과하나 데이터 필터(`device_config.site_id` 단독)는 신규 `Shingi` 데이터를 잡지 못함을 확인
* [x] **업로더 v5.4 정상 동작 확인 (초기 판단 정정)**: 구버전 설치로 추정했으나, 약 15분 뒤 재조회 시 `sync_device_config_to_supabase()`가 다음 동기화 주기에 스스로 교정한 것을 확인. exe 교체 불필요
* [x] **자동 교정 범위의 한계 식별**: v5.4는 `device_config`만 교정하며, 이미 옛 `Site_ID`로 적재된 측정 데이터는 그대로 남는다는 점을 명확화

#### 23-2. 데이터 통합 실행
* [x] **`device_config.site_id` → `Shingi`** (1건)
* [x] **과거 측정 데이터 1,265건의 `Site_ID` 통합**: `TMSTOC-250701-01` → `Shingi`. 2025-06-30 ~ 2026-08-13 전 구간 연결 (통합 후 `Shingi` 1,269건)
* [x] **사전 안전 확인**: 옛 Site_ID를 쓰는 타 기기 데이터 0건 확인 후 실행, UPDATE 필터를 `Device_ID` 로 한정하여 타 지점 무영향

#### 23-3. 산출물 및 문서화
* [x] **`tools/migrate_site_id.py` 신설**: `device_config` 교정 + 과거 데이터 통합을 수행하는 멱등 스크립트. 타 기기 데이터 혼입 시 중단 가드 내장. 재실행으로 멱등성 검증
* [x] **`docs/ARCHITECTURE.md` 트러블슈팅 표 확장**: "사이트 이름으로는 접속되는데 site_id로는 막힘", "최근 데이터만 보이고 과거가 끊김" 2개 증상 행 추가
* [x] **배포 패키지 config에 Supabase 접속 정보 기입**: `계측기_PC_배포패키지_v5.4/uploader_config.json`에 URL/Key 기입 및 `is_mock=false`. Fail-safe 유지를 위해 `site_id`/`site_name`/`is_paused`는 `auto`/`true` 유지. `.gitignore`로 저장소 미커밋 확인

#### 23-4. 미처리 (사용자 판단)
* [ ] **`TOC-260706-03`의 `site_id`가 `TOC-260706-02`(타 기기 지점 ID)** — 두 기기 모두 현장 가동 전이고 데이터 문제 없어 **그대로 두기로 결정**. 현장 설치 시 업로더 설정으로 v5.4가 자동 교정

---

### 24. NAS 인계 수기 문서 최신화 (2026-09-10)

대상: `Z:\…\05. TOC850_웹대시보드\04_운영관리_인프라정보.md` · `05_아키텍처_구조_명세서.md`
(둘 다 `tools/nas_sync.py` 의 `PRESERVED` 항목이라 자동 갱신되지 않는다. 최종 수정 2026-07-09에서 멈춰 있었다.)
상세 내역은 `meeting_notes/2026-09-10_인계문서_최신화.md` 참조.

#### 24-1. 사실 확인 (저장소 · 실제 서버)
* [x] **업로더 버전 v5.2 → v5.5 확인**: `build_release.py` `VERSION` · `gui_uploader.py` `APP_VERSION`. 원격 정지·주기 조정·자가 업데이트는 커밋 `fa8f5f7`
* [x] **현장 기기 5대 확인**: `device_config` 직접 조회 (jeomchon / PYEONGTAEK_MAIN / Pyeongtaek_pump / Samyang_Incheon / Shingi)
* [x] **실제 테이블 스키마 확인**: `device_config` 에 `is_active` 는 실재하지 않음. `measure_logs_v2` PK 는 `Date_Time` 이 아니라 `id`
* [x] **원격 관리 마이그레이션 미적용 확인**: `remote_paused` 등 4개 컬럼 `42703`, `uploader_release` 테이블 `PGRST205`. **v5.5 원격 관리는 현재 동작하지 않는다**
* [x] **`4ade1f9` 고정 원칙 폐기 확인**: 9/6~9/7 커밋이 `main` 병합·배포됨 (`origin/main` = `4ebd948`)
* [x] **번들 내 `service_role` 노출 재확인 — 2곳이 아니라 3곳**: 고객용·사무실용에 더해 **테스트 배포 `dashboard-ochre-two-35`** 도 포함(접근 제한 없음)
* [x] **문서에 적힌 anon 키가 무효임을 확인**: `401 Invalid API key`. 서명부 42자로 전사 손상 추정 → 콘솔 재복사 필요
* [x] **텔레그램 웹훅 실제 등록 주소 확인**: 실서버가 아닌 **테스트 배포**(`getWebhookInfo`). 봇 사용자명은 `@laskorea_alert_bot`

#### 24-2. 문서 갱신
* [x] **`04_운영관리_인프라정보.md`**: 보안 경고(§0) 신설, Supabase 요금제 Pro·스키마·키 경고, Vercel 3개 배포 및 환경변수 구분, 텔레그램 정보 정정, 현장 5대 표, 형제 프로젝트(§5)·NAS 동기화(§6)·확인 목록(§7)·근거(§8) 신설. **키 값은 새로 적지 않고 기존 값 유지 + 경고만 추가**
* [x] **`05_아키텍처_구조_명세서.md`**: ERD 실제 스키마로 교정, v5.5 기준 적재 시퀀스(원격 제어·전체 재업로드 방지 분기), 원격 관리·자가 업데이트 시퀀스(§4), 대시보드 조회 경로(§5), 보안 구조(§6) 신설
* [x] **원본 백업**: NAS `_보존/*.20260910_213303.md`
* [x] **보존 확인**: `python tools/nas_sync.py --dry-run` 에서 두 문서 모두 `(보존)` 으로 표시 — 자동 동기화가 덮어쓰지 않음

#### 24-3. 미처리 (확인 필요)
* [ ] **RLS 활성화 여부** — 문서의 anon 키가 무효라 검증 불가. 콘솔에서 anon 키 재취득 후 `docs/2026-09-10_보안_및_트래픽_후속작업_인계.md` §1.3 명령으로 확인
* [ ] **Vercel 3개 프로젝트의 실제 환경변수 목록·값** — CLI 미로그인 (`npx vercel env ls`)
* [ ] **Supabase Database Webhook 의 등록 대상 URL**(실서버/테스트) — 콘솔에서만 확인 가능
* [ ] **현장 PC 에 실제 설치된 업로더 버전** — v5.5 현장 반입 기록 없음
* [ ] **NAS 폴더 권한이 개발부로 제한되어 있는지**
* [ ] **`docs/2026-09-10_보안_및_트래픽_후속작업_인계.md` §1.1 의 "번들 2곳" 기술을 3곳으로 보정**
* [ ] **`.agents/AGENTS.md` §3(2026-07-07 기준) 갱신** — 이번 작업 범위 밖
