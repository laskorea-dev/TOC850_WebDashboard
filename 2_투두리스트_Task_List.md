...
### Phase 18: Vercel 직접 텔레그램 발송 튜닝, 접속 패스코드 변경 및 수신처 삭제 권한 완화 (완료 100%)
* [x] **Vercel(웹 브라우저)에서 직접 텔레그램 Bot API를 호출하여 즉시(0초 딜레이) 테스트 메시지를 보내는 `sendTelegramAlert` 연동**
* [x] **알림 수신인 등록/삭제 시 텔레그램 봇이 환영/작별 메시지를 즉시 발송하도록 튜닝**
* [x] **설정창(설정 모달) 내부의 수신자 삭제(`🗑️ 삭제`) 권한을 어드민 여부와 무관하게 상시 작동하도록 완화**
* [x] **설정 모달 및 어드민 설정 카드 내에 대시보드 접속 패스코드(비밀번호) 변경 텍스트 입력창 신설 및 DB PATCH 동기화**
* [x] **설정 모달 및 어드민 설정 카드 내에 자체 텔레그램 봇 토큰 입력 필드 추가 및 `toc_alert_high` JSON 필드 연동 저장**
* [x] **계측기 GUI Uploader 백그라운드 스레드에서 Supabase `device_config`를 읽을 때 웹에서 설정한 `telegram_bot_token`을 자동 감지하여 동기화**

---

### Phase 19: 설정창 UI 레이아웃 콤팩트 고도화 및 텔레그램 봇 자가 등록 (/등록) 구현 (완료 100%)
* [x] **대시보드 메인 헤더의 `🔔 알림 수신 등록` 버튼을 삭제하고, 설정 모달 내부 수신자 목록 타이틀 옆에 수신처 추가 단추 매립**
* [x] **설정 모달 및 어드민 설정 카드 내의 투박한 채널 테이블을 반응형 Grid Layout 기반의 콤팩트 카드형 배치로 전환**
* [x] **설정 모달창의 가로폭을 800px로 넓히고, 모달 몸체에 세로 스크롤(`max-height: 90vh`, `overflow-y: auto`) 적용하여 사용성 강화**
* [x] **설정 모달 내에 중복 노출되어 세로 공간을 낭비하던 이메일/봇토큰/챗ID 입력창들을 전면 삭제하고 등록인 목록만 깔끔하게 유지**
* [x] **일반 공개 등록 모달에서 이메일 등록 옵션을 삭제하고 오직 텔레그램 가이드와 Chat ID 입력창만 활성화되도록 단독 모드 고정**
* [x] **사용자가 텔레그램 봇 대화방에서 `/등록 [디바이스ID] [사용자명] [패스코드]`를 입력하면, Vercel Serverless Function(Edge API Webhook)이 즉시 기기 비밀번호를 검증하고 Supabase DB에 수신자를 자동 자가등록하는 서버리스 Webhook 아키텍처 구현 완료**
...
* [x] **트렌드 차트(Recharts) 내 이중 Y축(보조축) 채널 선택 기능 구현 및 설정 UI 레이아웃 사소한 문구 정리 완료** 및 데이터베이스 저장 기반의 영구 채널 축 설정(기본축/보조축 지정)으로 전환 완료

---

### Phase 20: 텔레그램 스마트 자가등록 및 대시보드 UI 장비 ID 개선 (완료 100%)
* [x] **설정 모달 창의 알림 수신자 헤더 옆 장비 ID 노출부에 폴백(Fallback) 감지 추가**: 로그 상의 Device_ID가 사이트 ID와 같으면 DB 설정의 `device_id` 필드를 노출시켜 장비 번호를 정상 노출하도록 조치
* [x] **텔레그램 알림 봇 등록 Webhook (`/api/telegram-webhook.js`) 쿼리에 OR 다중 검색 튜닝**: 사용자가 기기 ID 또는 사이트 ID 중 어느 것을 입력하더라도 매칭 성공하도록 PostgREST `or` 쿼리 적용
* [x] **실서비스 도메인(`toc-850-web-dashboard.vercel.app`) 및 테스트 스테이징 서버 최신 리비전 통합 릴리즈 및 원격 Git 릴리즈 배포 완수**

---

### Phase 21: 차트 기본축 및 보조축 스케일 범위 지정 및 동적 확장 (완료 100%)
* [x] **기본축(Left YAxis) 및 보조축(Right YAxis) 스케일 범위 자동 설정**: 0을 하한으로 하고 해당 축에 설정된 표시 채널의 경고치(Warning Threshold) 중 최대값을 상한으로 하는 기본 스케일 산정 알고리즘 구축
